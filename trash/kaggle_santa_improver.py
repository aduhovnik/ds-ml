		...
		if (1) {
			...
			for (i = 5001; i < 45001; i += 2) {
				...
				for (j = 0; j < 1000; j++) {
					if (preds[i] == j)
						continue;
					cur_score = try_update_twins(4 * ITERS_PER_UPDATE, i, j, &tch, &tsh, &cur_score);
				}
			}
		...
		if (1) {
			...
			for (i = 45001; i < 1000000; i++) {
				...
				for (j = 0; j < 1000; j++) {
					...
					cur_score = try_update_singles(5*ITERS_PER_UPDATE, i, j, &tch, &tsh, &cur_score);
				}
			}
...
if __name__ == '__main__':
    initial_sub = '../input/try-santa-06/subm_0.935650335404.csv'
